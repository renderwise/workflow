 ...
 */
@WebServlet("/logincheckdoctor")
public class logincheckdoctor extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public logincheckdoctor() {
		super();
			...
			if (cidarr.contains(Case_Id)) {
				...
				if (plnarr.contains(plan_no)) {
					String sqlpn = "SELECT Patient_Name,Doctor_Name FROM patient_details where Case_Id='" + Case_Id
							+ "'";
					ResultSet rscd = st2.executeQuery(sqlpn);

					String pname, dname;
					
					while (rscd.next()) {
						pname = rscd.getString("Patient_Name");
						dname = rscd.getString("Doctor_Name");
					...
					String sqlstct = "SELECT * FROM CATSTART where Case_Id='" + Case_Id + "' and plan_no='"
							+ plan_no + "'";
					ResultSet rsstct = st2.executeQuery(sqlstct);

					String st,ct;

					while (rsstct.next()) {
						st = rsstct.getString("starter_kit");
						ct = rsstct.getString("category");
